...
const handler = async (m, { conn, usedPrefix }) => {
  if (global.conn.user.jid !== conn.user.jid) {
    return conn.sendMessage(m.chat, {text: '⚠️ Este comando solo puede ser utilizado en el bot principal de Sԋιɳιɳɠ Mσσɳ.'}, {quoted: m});
  }
  await conn.sendMessage(m.chat, {text: '✅ Iniciando...'}, {quoted: m});
  const sessionPath = './sessions'
  try {
    if (!existsSync(sessionPath)) {
      return await conn.sendMessage(m.chat, {text: '⚠️ La carpeta session de Sԋιɳιɳɠ Mσσɳ no existe o está vacía.'}, {quoted: m});
    }
